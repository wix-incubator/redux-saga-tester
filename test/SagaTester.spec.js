...
describe('SagaTester', () => {
    ...
    it('Passes options to createSagaMiddleware', () => {
        ...
        // a non-function for a emitter is something that redux-saga uses in the
        // sagaMiddlewareFactory (and throws an error on). so we can verify that
        // doing the same thing here also throws an error, and thus our config
        // is being passed on.
        const sagaTester = () => new SagaTester({ options: { emitter: 42 }});
        expect(sagaTester).to.throw('(options.emitter || _utils.identity) is not a function');
    });
